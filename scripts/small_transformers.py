#!/usr/bin/env python3
"""
Small-Scale Pretraining Script for Standard Multi-Head Attention Transformer
Test on RTX 3050 single GPU before scaling to multi-GPU setup

Standard Transformer Architecture:
- Multi-head self-attention with O(n²) complexity
- Positional embeddings for sequence understanding
- Layer normalization and residual connections
- Feed-forward networks with GELU activation
- Causal masking for autoregressive generation

EVOLVING POSITIONAL EMBEDDINGS SUPPORT:
- Revolutionary infinite context capability
- Content-aware dynamic positional encoding
- Outperforms fixed embeddings on language modeling
...
# Import our revolutionary evolving positional embeddings
import sys
sys.path.append('..')
from evolving_positional_embeddings import EvolvingPositionalEmbedding, EvolvingMultiHeadAttention, EvolvingTransformerLayer
print("🌟 Evolving Positional Embeddings - The Future of Transformers!")
try:
    import wandb
...
# Use the EvolvingTransformerLayer from evolving_positional_embeddings.py directly

# Legacy standard attention removed - evolving embeddings are the future! 🌟

# Legacy standard language model removed - evolving model is the only way! ♾️

class LanguageModel(nn.Module):
    """
    🌟 Revolutionary Evolving Transformer Language Model
    - Infinite context capability with evolving positional embeddings
    - Content-aware dynamic positional encoding
    - Outperforms fixed embeddings on language modeling
    - No sequence length limitations
    """
    def __init__(self, vocab_size, d_model=512, n_heads=8, n_layers=6, d_ff=2048, 
        ...
        # Token embedding (no positional embedding needed!)
        self.token_embedding = nn.Embedding(vocab_size, d_model)
        self.dropout = nn.Dropout(dropout)
        
        # Revolutionary evolving transformer blocks
        self.blocks = nn.ModuleList([
            EvolvingTransformerLayer(d_model, n_heads, d_ff, dropout)
        ...
        print(f"🌟 Created Evolving Language Model with INFINITE context capability!")
        print(f"   📊 Parameters: {sum(p.numel() for p in self.parameters()):,}")
        print(f"   🚀 No sequence length limitations!")
    ...
    def forward(self, input_ids, targets=None):
        ...
        # Token embeddings (no positional embeddings needed!)
        x = self.token_embedding(input_ids)
        x = self.dropout(x)
        
        # Apply evolving transformer blocks (they handle positional encoding internally)
        for block in self.blocks:
            x = block(x, mask=True)  # Always use causal mask for language modeling
    ...
    def forward(self, input_ids, targets=None):
        ...
        mask = self.causal_mask[:, :, :T, :T] if T <= self.max_seq_len else self._create_causal_mask(T).to(input_ids.device)
...
def train_epoch(model_engine, dataloader, device, epoch, config, global_step=0, best_val_loss=float('inf')):
    ...
    for batch_idx, (input_ids, targets) in enumerate(dataloader):
        ...
        if current_step % config['eval_every'] == 0 and current_step > 0:
            ...
            # Track best model (just log, don't save during training to avoid I/O bottlenecks)
            if val_loss < best_val_loss:
                best_val_loss = val_loss
                logger.info(f"🏆 Best validation loss updated to {val_loss:.4f} at step {current_step} (rank {deepspeed.comm.get_rank()})")
...
def main():
    ...
    config = {
        ...
        'val_split': 0.1,
        'logging_steps': 50,   # Log every step to wandb
    ...
    # Create model - Always use Evolving Positional Embeddings
    logger.info("🌟 Creating EVOLVING Transformer model with infinite context...")
    model = LanguageModel(
    ...
    # Training loop
    logger.info("🚀 STARTING PRETRAINING OF STANDARD TRANSFORMER!")
    logger.info("=" * 60)
    ...
    if global_step > 0:
        ...
        # Calculate how many complete epochs we've done and remaining steps
        steps_per_epoch = len(train_dataloader)
        completed_epochs = resume_step // steps_per_epoch
        remaining_steps_in_epoch = resume_step % steps_per_epoch
        start_epoch = completed_epochs
        
        logger.info(f"🔄 Resuming from step {resume_step}: completed {completed_epochs} epochs, {remaining_steps_in_epoch} steps into epoch {start_epoch + 1}")
        
        # Calculate remaining steps to complete training
        ...
        best_val_loss = float('inf')  # Initialize best validation loss for fresh training
