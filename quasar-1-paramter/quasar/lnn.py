...
class LNNCell(nn.Module):
    """
    A single Liquid Neural Network cell. The internal weights are fixed,
    and the dynamics are controlled by a single external parameter.
    """
    def __init__(self, config: LNNConfig):
        ...
        self.W = nn.Parameter(torch.empty(config.hidden_size, config.hidden_size), requires_grad=False)
        self.U = nn.Parameter(torch.empty(config.hidden_size, config.hidden_size), requires_grad=False)
        self.b = nn.Parameter(torch.empty(config.hidden_size), requires_grad=False)

        self.tau_w_h = nn.Linear(config.hidden_size, config.hidden_size)
        self.tau_w_u = nn.Linear(config.hidden_size, config.hidden_size)
        for p in self.tau_w_h.parameters():
            p.requires_grad = False
        for p in self.tau_w_u.parameters():
            p.requires_grad = False
            
        self.tau_b = nn.Parameter(torch.empty(config.hidden_size), requires_grad=False)
    ...
    def forward(self, h: torch.Tensor, u: torch.Tensor, lambda_param: torch.Tensor) -> torch.Tensor:
        """Core ODE dynamics, modulated by the single learnable lambda_param."""
        with torch.no_grad():
        ...
        dx_dt_base = decay_term + activation_output

        # The single learnable parameter scales the dynamics
        positive_lambda = F.softplus(lambda_param)
        dx_dt = positive_lambda * dx_dt_base
...
class LNNBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h: torch.Tensor, lambda_param: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Processes the entire sequence, passing lambda_param to the cell.
        """
        seq_len = x.size(1)
        ...
        for t in range(seq_len):
            u = x[:, t, :]
            dx_dt = self.cell(h, u, lambda_param)
            h = h + self.dt * dx_dt
            outputs[:, t, :] = h
...
class LNNModel(PreTrainedModel):
    ...
    def __init__(self, config: LNNConfig):
        ...
        # --- The Single Learnable Parameter for the recurrent core ---
    ...
    def forward(
        ...
        **kwargs,  # Accept other arguments
        ...
        for i, block in enumerate(self.blocks):
            ...
            # Pass the single learnable parameter to each block
            block = cast(LNNBlock, block) # for typechecker
            layer_output, h_final = block(layer_output, h_initial, self.lambda_param)
